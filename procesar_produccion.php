...
if ($userIDResult->num_rows > 0) {
    ...
    $existingSql = "SELECT * FROM produccion_diaria WHERE usuario_id = $usuario_id AND fecha = '$fecha'";
    $result = $conn->query($existingSql);

    if ($result->num_rows > 0) {
        // La producción ya existe para esa fecha y usuario, realizar una actualización
        $updateSql = "UPDATE produccion_diaria SET metros_cubicos = $metrosCubicos WHERE usuario_id = $usuario_id AND fecha = '$fecha'";

        if ($conn->query($updateSql) === TRUE) {
            $_SESSION['produccion_actualizada'] = true; // Almacenar en sesión que la producción se ha actualizado
            header("Location: produccion_diaria.html"); // Redirigir a produccion_diaria.html
            exit();
        } else {
        ...
        $insertSql = "INSERT INTO produccion_diaria (usuario_id, fecha, metros_cubicos) VALUES ($usuario_id, '$fecha', $metrosCubicos)";
